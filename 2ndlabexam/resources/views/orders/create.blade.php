<x-app-layout>
    ...
    <div class="page-header">
        <div>
            ...
            <p>Fill in the details below to place a new order.</p>
        </div>
    ...
    <div class="table-wrap" style="max-width: 640px;">
        <form method="POST" action="{{ route('orders.store') }}" style="padding: 8px 0;">
            @csrf
                ...
                <select name="rice_id" id="rice_id" class="form-control" required>
                    ...
                    @foreach($rices as $rice)
                        <option value="{{ $rice->id }}"
                            ...
                            data-stock="{{ $rice->stock }}"
                            {{ old('rice_id') == $rice->id ? 'selected' : '' }}>
                            {{ $rice->name }} — ₱{{ number_format($rice->price_per_kg, 2) }}/kg &nbsp;(Stock: {{ $rice->stock }} kg)
                        </option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="form-group">
                ...
                <input type="number" name="quantity" id="quantity" class="form-control"
                    step="0.1" min="0.1" value="{{ old('quantity') }}" required placeholder="0.0">
            </div>

            <div class="price-summary">
                <div class="price-row">
                    <span>Price per kg</span>
                    <span id="price-per-kg" style="font-weight:600; color:var(--brown);">₱0.00</span>
                </div>
                <div class="price-row">
                    <span>Total Cost</span>
                    <span id="total-cost" style="font-size:18px; font-weight:700; color:var(--brown);">₱0.00</span>
                </div>
            </div>
